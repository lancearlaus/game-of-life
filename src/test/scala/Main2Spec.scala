...
class Main2Spec extends WordSpec with Matchers {
  ...
  "readState" should {
    "read string state" in {
      ...
      val expected = State(Set.empty[Cell], Map(events: _*))
  ...
  "nextState" should {
    ...
    "produce correct next state" in {
      ...
      val expectedCells = readState(expectedState).events.keySet
      val actualCells = nextState(nextState(initial, bounds), bounds).cells
